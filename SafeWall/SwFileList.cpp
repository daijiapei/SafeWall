...
MY_LIST_HERDER gFileListHeader;          //加密文件链表


#pragma PAGEDCODE 
VOID InitializeMyFileListHead()
{
	InitializeListHead((PLIST_ENTRY)&gFileListHeader);//初始化文件链表
	KeInitializeSpinLock(&gFileListHeader.locker);
	gFileListHeader.icount = 0;
}



#pragma PAGEDCODE 
VOID ReleaseMyFileListHead()
{
	return ; //什么也不做
}
